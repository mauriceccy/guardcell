...
import numpy
SucroseList = numpy.arange(0.0, 2.0, 0.01)
mat = scobra.matrix()
for n in SucroseList:
	m.SetConstraint('Sucrose_tx_Day', n, n)
	m.MinFluxSolve()
	sol = m.GetSol(IncZeroes=True)
	mat = mat.UpdateFromDic(sol)
vr = scobra.matrix(mat).VaryReacs()
mat.T.to_csv('GC_sucrose_scan_standard.csv',header=False)
vr.T.to_csv('GC_sucrose_scan_standard_vary.csv',header=False)
