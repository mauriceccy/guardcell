...
m.PrintSol('Photon')
m.PrintSol('CO2_tx')
sol.AsMtx().ToFile('GC_Cl_only.csv')

fva = m.FVA()
fva.AsMtx().to_csv('GC_Cl_only_FVA.csv')
...
#restrict malate
m.SetConstraint('Malate_OpenDay_storage', 0, None)
m.SetConstraint('Malate_DayClose_storage', 0, None)
m.SetConstraint('Malate_CloseNight_storage', 0, None)
m.SetConstraint('Malate_NightOpen_storage', 0, None)
m.MinFluxSolve()
standard = m.GetSol(IncZeroes=True)

diff = scobra.flux(sol.Diff(standard))
diff.AsMtx().to_csv('GC_Cl_malate_diff.csv')
